...
use alloy::primitives::{keccak256, Address as AlloyAddress, FixedBytes, U256 as AlloyU256};
...
pub struct Eip7683Config {
	...
	/// OutputSettler addresses for each chain
	pub output_settler_addresses: Vec<Address>,
...
impl Default for Eip7683Config {
	fn default() -> Self {
		Self {
			...
			supported_chains: vec![31337, 31338], //  Hardhat
			...
			output_settler_addresses: vec![],                                         // Must be configured
...
impl OrderPlugin for Eip7683OrderPlugin {
	...
	async fn parse_order(&self, data: &[u8]) -> PluginResult<Self::Order> {
		...
		let resolved_order_type = DynSolType::Tuple(vec![
			DynSolType::Address,        // user
			DynSolType::Uint(256),      // originChainId
			DynSolType::Uint(256),      // openDeadline (stored as uint256 in ABI encoding)
			DynSolType::Uint(256),      // fillDeadline (stored as uint256 in ABI encoding)
			DynSolType::FixedBytes(32), // orderId
			DynSolType::Array(Box::new(DynSolType::Tuple(vec![
				// maxSpent (Output[])
				DynSolType::FixedBytes(32), // token (bytes32)
				DynSolType::Uint(256),      // amount
				DynSolType::FixedBytes(32), // recipient (bytes32)
			...
			DynSolType::Array(Box::new(DynSolType::Tuple(vec![
				// fillInstructions
				DynSolType::Uint(256),      // destinationChainId
				DynSolType::FixedBytes(32), // destinationSettler (bytes32)
				DynSolType::Bytes,          // originData
			]))),
		]);

		// Decode the event data
		// The event data already includes the offset, so we decode the entire data
		let decoded_value = resolved_order_type.abi_decode(data).map_err(|e| {
			PluginError::ExecutionFailed(format!("Failed to decode event data: {}", e))
		})?;

		// Extract the struct from the decoded data
		let resolved_order = match &decoded_value {
			DynSolValue::Tuple(fields) if fields.len() == 8 => fields,
			...
			_ => {
				return Err(PluginError::ExecutionFailed(
					"Expected tuple value for ResolvedCrossChainOrder".to_string(),
				));
			}
		};
		...
		let origin_chain_id = match &resolved_order[1] {
			DynSolValue::Uint(chain_id, _) => chain_id.to::<u64>(),
			_ => {
				return Err(PluginError::ExecutionFailed(
		...
		let open_deadline = match &resolved_order[2] {
			DynSolValue::Uint(deadline, _) => deadline.to::<u64>(),
			_ => {
				return Err(PluginError::ExecutionFailed(
		...
		let max_spent = match &resolved_order[5] {
			DynSolValue::Array(outputs) => outputs
				.iter()
				.map(|output| match output {
					DynSolValue::Tuple(fields) => {
						...
						let amount = match &fields[1] {
							DynSolValue::Uint(amt, _) => amt.to::<u64>(),
							_ => {
								return Err(PluginError::ExecutionFailed(
						...
						let chain_id = match &fields[3] {
							DynSolValue::Uint(chain, _) => chain.to::<u64>(),
							_ => {
								return Err(PluginError::ExecutionFailed(
		...
		let min_received = match &resolved_order[6] {
			DynSolValue::Array(inputs) => inputs
				.iter()
				.map(|input| match input {
					DynSolValue::Tuple(fields) => {
						if fields.len() >= 4 {
							...
							let token = match &fields[0] {
								DynSolValue::FixedBytes(bytes, _) if bytes.len() == 32 => {
									format!("0x{}", hex::encode(&bytes[12..32]))
								}
								_ => {
									return Err(PluginError::ExecutionFailed(
		...
		let destination_chain_id = match &resolved_order[7] {
			DynSolValue::Array(instructions) => instructions
				.first()
				.and_then(|instruction| match instruction {
					DynSolValue::Tuple(fields) => match &fields[0] {
						DynSolValue::Uint(chain_id, _) => Some(chain_id.to::<u64>()),
						_ => None,
					},
					_ => None,
				})
		...
		let order = Eip7683Order {
			...
			created_at: std::time::SystemTime::now()
				...
				.map_err(|e| PluginError::ExecutionFailed(format!("System time error: {}", e)))?
				.as_secs(),
	...
	async fn create_fill_request(&self, order: &Self::Order) -> PluginResult<DeliveryRequest> {
		...
		let settler_hex = self
			.config
			.output_settler_addresses
			.first()
			.ok_or_else(|| {
				PluginError::ExecutionFailed("No output settler address configured".to_string())
			})?
		...
		let _mandate_output_type = DynSolType::Tuple(vec![
			DynSolType::FixedBytes(32), // oracle
			DynSolType::FixedBytes(32), // settler
			DynSolType::Uint(256),      // chainId
			DynSolType::FixedBytes(32), // token
			DynSolType::Uint(256),      // amount
			DynSolType::FixedBytes(32), // recipient
			DynSolType::Bytes,          // call
			DynSolType::Bytes,          // context
		]);
		let mandate_output_value = DynSolValue::Tuple(vec![
			DynSolValue::FixedBytes(
				FixedBytes::<32>::try_from(oracle_bytes32.as_slice()).map_err(|_| {
					PluginError::ExecutionFailed("Invalid oracle bytes32 length".to_string())
				})?,
				32,
			),
			DynSolValue::FixedBytes(
				FixedBytes::<32>::try_from(settler_bytes32.as_slice()).map_err(|_| {
					PluginError::ExecutionFailed("Invalid settler bytes32 length".to_string())
				})?,
				32,
			),
			DynSolValue::Uint(AlloyU256::from(destination_output.chain_id), 256),
			DynSolValue::FixedBytes(
				FixedBytes::<32>::try_from(token_bytes32.as_slice()).map_err(|_| {
					PluginError::ExecutionFailed("Invalid token bytes32 length".to_string())
				})?,
				32,
			),
			DynSolValue::Uint(AlloyU256::from(destination_output.amount), 256),
			DynSolValue::FixedBytes(
				FixedBytes::<32>::try_from(recipient_bytes32.as_slice()).map_err(|_| {
					PluginError::ExecutionFailed("Invalid recipient bytes32 length".to_string())
				})?,
				32,
			),
		...
		let origin_data = mandate_output_value.abi_encode();
		...
		let solver_address = AlloyAddress::parse_checksummed(&self.config.solver_address, None)
			.map_err(|e| PluginError::ExecutionFailed(format!("Invalid solver address: {}", e)))?;
		filler_data[12..32].copy_from_slice(solver_address.as_slice());
		...
		// Encode the parameters together for proper function call encoding
		// Create a function type that matches fill(bytes32,bytes,bytes)
		let params = vec![
			DynSolValue::FixedBytes(
				FixedBytes::<32>::try_from(order_id_bytes.as_slice()).map_err(|_| {
					PluginError::ExecutionFailed("Invalid order ID bytes32 length".to_string())
				})?,
				32,
			),
			DynSolValue::Bytes(origin_data),
			DynSolValue::Bytes(filler_data),
		];

		// Create a tuple of the parameters and encode it without the tuple wrapper
		// This is equivalent to ethers' encode(&tokens)
		...
		let output_settler =
			AlloyAddress::parse_checksummed(&self.config.output_settler_addresses[0], None)
				.map_err(|e| {
					PluginError::ExecutionFailed(format!("Invalid output settler address: {}", e))
				})?;

		let transaction = Transaction {
			to: format!("{:?}", output_settler), // Use validated address
	...
	async fn create_settlement_request(
		...
		// Create the finaliseSelf function call using Alloy's dynamic ABI

		// Define the finaliseSelf function signature and get selector
		...
		// Parse addresses - use regular parse for external data that might not be checksummed
		let user_address = order.user.parse::<AlloyAddress>().unwrap_or_else(|e| {
			tracing::error!("Failed to parse user address {}: {}", &order.user, e);
			AlloyAddress::default()
		});
		let local_oracle_address = order
			.local_oracle
			.parse::<AlloyAddress>()
			.unwrap_or_else(|e| {
				tracing::error!(
					"Failed to parse oracle address {}: {}",
					&order.local_oracle,
					e
				);
				AlloyAddress::default()
		...
		let input_settler_address = self
			...
			.input_settler_addresses
			.first()
			.and_then(|s| AlloyAddress::parse_checksummed(s, None).ok());
		let output_settler_address = self
			...
			.and_then(|s| AlloyAddress::parse_checksummed(s, None).ok());
		let solver_address =
			AlloyAddress::parse_checksummed(&self.config.solver_address, None).unwrap_or_default();

		// Encode the order struct
		let order_struct = DynSolValue::Tuple(vec![
			DynSolValue::Address(user_address),
			DynSolValue::Uint(AlloyU256::from(order.nonce), 256),
			DynSolValue::Uint(AlloyU256::from(order.origin_chain_id), 256),
			DynSolValue::Uint(AlloyU256::from(order.expires_at), 256), // uint32 encoded as U256
			DynSolValue::Uint(AlloyU256::from(order.fill_deadline), 256), // uint32 encoded as U256
			DynSolValue::Address(local_oracle_address),
			DynSolValue::Array(if order.inputs.is_empty() {
				...
				order
					...
					.map(|output| {
						...
						let token_u256 = AlloyU256::from_be_bytes(
							token_u256_bytes.try_into().unwrap_or_else(|_| {
								tracing::error!("Failed to convert token bytes to [u8; 32]");
								[0u8; 32]
							}),
						);

						DynSolValue::FixedArray(vec![
							DynSolValue::Uint(token_u256, 256),
							DynSolValue::Uint(AlloyU256::from(output.amount), 256),
						])
					})
					.collect()
			} else {
				...
				order
					...
					.map(|output| {
						DynSolValue::Tuple(vec![
							DynSolValue::FixedBytes(FixedBytes::<32>::from([0u8; 32]), 32), // oracle - zero for same-chain
							DynSolValue::FixedBytes(
								{
									...
									if let Some(output_settler_addr) = output_settler_address {
										settler_bytes[12..32]
											.copy_from_slice(output_settler_addr.as_slice());
									}
									FixedBytes::<32>::try_from(settler_bytes.as_slice())
										.unwrap_or_else(|_| {
											tracing::error!("Invalid settler bytes32 length");
											FixedBytes::<32>::from([0u8; 32])
										})
								},
								32,
							),
							DynSolValue::Uint(AlloyU256::from(output.chain_id), 256),
							DynSolValue::FixedBytes(
								{
									let mut token_bytes = vec![0u8; 32];
									let token_addr =
										output.token.parse::<AlloyAddress>().unwrap_or_else(|e| {
											tracing::error!(
									...
									FixedBytes::<32>::try_from(token_bytes.as_slice())
										.unwrap_or_else(|_| {
											tracing::error!("Invalid token bytes32 length");
											FixedBytes::<32>::from([0u8; 32])
										})
								},
								32,
							...
							DynSolValue::FixedBytes(
								{
									...
									recipient_bytes[12..32]
										.copy_from_slice(recipient_addr.as_slice());
									FixedBytes::<32>::try_from(recipient_bytes.as_slice())
										.unwrap_or_else(|_| {
											tracing::error!("Invalid recipient bytes32 length");
											FixedBytes::<32>::from([0u8; 32])
										})
								},
								32,
		...
		let timestamps = DynSolValue::Array(vec![DynSolValue::Uint(
			AlloyU256::from(fill_timestamp as u32),
			256, // uint32 encoded as U256
		)]);

		// Create solver bytes32 from the configured solver address
		let solver_token = DynSolValue::FixedBytes(
			{
				let mut solver_bytes = vec![0u8; 32];
				solver_bytes[12..32].copy_from_slice(solver_address.as_slice());
				FixedBytes::<32>::try_from(solver_bytes.as_slice()).map_err(|_| {
					PluginError::ExecutionFailed("Invalid solver bytes32 length".to_string())
				})?
			},
			32,
		);
		...
		let input_settler_addr = input_settler_address.ok_or_else(|| {
			PluginError::ExecutionFailed(
				"No valid InputSettler address configured for origin chain".to_string(),
			)
		})?;

		// Create the settlement transaction
		let transaction = Transaction {
			to: format!("{:?}", input_settler_addr), // Use validated address
